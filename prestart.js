...
let myOptionKey = 'assist-scale-enemy-hp';
...
sc.ScaleEnemyHpMod = ig.GameAddon.extend({
  factor: 0,
...
sc.GameModel.inject({
  isAssistMode() {
    ...
    // implementation.
    return this.parent() || sc.scaleEnemyHpMod.factor != 1;
  },
});

sc.EnemyType.inject({
  onload(data) {
    let mod = sc.scaleEnemyHpMod;
...
ig.addGameAddon(() => {
  let addon = new sc.ScaleEnemyHpMod();
  sc.scaleEnemyHpMod = addon;
  return addon;
});
